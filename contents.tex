...
In some biological research projects, we encounter problems that should be studied by measuring fluorescence intensity at the boundary between two different compartments. Here, we pick up an example analysis that measures the changes in nuclear pore complex (NPC) protein density at nuclear membrane over time, in order to study the translocation process the protein from the cytoplasmic fraction to the nuclear membrane (ref). 
 ...
 We analyze a two-channel time-lapse image stack, a sequence of the process of NPC protein relocalizing from the cytoplasm to the nuclear membrane.  The data was acquired by Andreas Boni (Jan Ellenberg lab, EMBL Heidelberg) and have been used in many training. His work is published in The Journal of Cell Biology (Boni et al, 2016). The actual codes and image data used in this study are accessible through the journal website\footnote(URL). 
...
The image data we used in this chapter can be downloaded using the plugin "CMCI-EMBL". After installation of this plugin, select the menu item \ijmenu{[EMBL > Sample Images > NPCsingleCell.tif]} to load the image data. This is a time-lapse sequence of a cell, extracted from ``NPC1.tif'' which can be also downloaded through the same plugin. 
...
In the following we record these steps as macro commands using the Command Recorder (\ijmenu{[Plugins > Macros > Record\ldots]}). We recommend you NOT to launch the command recorder from the beginning. Please first try to reproduce the workflow using mouse and the graphic user interface (GUI). This is like a rehearsal before recording your actions. When you become clear with the steps you have to take, record the processing steps. When you use the command recorder, be sure that ``Macro'' is selected in the ``Record:'' drop down menu at the top-left corner of the recorder.
...
Standard behavior of ``Split Channel'' command is that it automatically names the resulting stacks of individual channels by prefixing ``C1-'' or ``C2-'' or ``C3'' to the original image title. Based on this known-behavior, We can construct these names ifthe original image title is known. For this we use the command \ilcom{getTitle()} which returns the title of currently active window as a string. 
...
The window title given in the argument of \ilcom{selectWindow} is hard-coded, so that if you need to apply this macro to a image data with a different widow title, it will not work. The macro
...
Using variables provided as the argument, calculation is done and then the answer is \textbf{return}ed. Functions normally has input and output. In this case, inputs are \ilcom{n} and \ilcom{m} and the output is the computation result. 
